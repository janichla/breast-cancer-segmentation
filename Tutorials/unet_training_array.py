...
def main(tempdir):
    ...
    for epoch in range(10):
        ...
        for batch_data in train_loader:
            step += 1
            # lädt jeweils ein Bild und eine Maske auf die GPU/CPU
            inputs, labels = batch_data[0].to(device), batch_data[1].to(device)
            # Zurücksetzen des Gradienten
            optimizer.zero_grad()
            # lässt Bilder durch das Modell laufen
            outputs = model(inputs)
            # berechnet neuen Verlust
            loss = loss_function(outputs, labels)
            # berechnet den Verlustgradienten
            loss.backward()
            # aktualisiert Modellparameter basierend auf dem berechneten Gradienten
            optimizer.step()
            # fügt Verlust zu, Gesamtverlust hinzu
